...
def install(md_name, branch='main', threads=4, text_only=True, specific_file=None, output=None, clean=True, check=True, max_retries=5):
    if md_name is None:
    ...
    try:
        model, branch = downloader.sanitize_model_and_branch_names(md_name, branch)
    ...
    links, sha256, is_lora, is_llamacpp = downloader.get_download_links_from_huggingface(
        model, branch, text_only=text_only, specific_file=specific_file
    )
    ...
    if check:
        ...
        downloader.download_model_files(
            model, branch, links, sha256, output_folder, specific_file=specific_file, threads=threads, is_llamacpp=is_llamacpp
        )
